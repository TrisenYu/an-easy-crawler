...
def refresh_token(cookie: str, token: str) -> None:
	...
	resp = getter(url=target, alter_dict={'Cookie': cookie})
	if resp is None:
		exit(1)
...
def refresh_dev_token(cook: str, token: str) -> None:
	...
	resp = poster(url=target, payload=enc_data, alter_dict={'Cookie': cook})
...
if __name__ == '__main__':
	...
	_token = PRIVATE_CONFIG['netease'][_args.refresh_dummy]["csrf_token"]
	_cookie = PRIVATE_CONFIG['netease'][_args.refresh_dummy]["cookie"]
