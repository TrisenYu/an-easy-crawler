...
from misc_utils import dic2ease_json_str, unix_ms
...
def just_crack_cookie() -> str:
	...
	inpt = dic2ease_json_str({
		"v": 'v1.1',
		"fp": f'{a}{netease_mmh32_checksum(f"{a}")},{b}{netease_mmh32_checksum(f"{b}")}',
		'u': f"{_f(_inject_str)}{unix_ms()}{_f(_inject_str)}",
		'h': 'music.163.com'
	})
